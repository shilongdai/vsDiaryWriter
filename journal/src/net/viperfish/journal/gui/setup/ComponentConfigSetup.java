...
public class ComponentConfigSetup extends ConfigView {
	...
	private final HashMap<String, JComboBox<String>> fields = new HashMap<String, JComboBox<String>>();

	private final ComponentConfig componentConfig;

	public ComponentConfigSetup(FirstTimeSetup firstTimeSetup,
			ComponentConfig componentConfig, boolean expert) {
		super(firstTimeSetup);
		this.componentConfig = componentConfig;
		setLayout(new MigLayout("", "[][][grow]", "[][][]"));

		JLabel lblNewLabel = new JLabel("Configuration For: "
				+ componentConfig.getUnitName());
	 ...
	 *            Configurations that will be added to the JPanel
	 */
		...
		for (String configurationItem : configurationItems) {
			...
			configuraitonItemOptionBox.addItemListener(new ItemListener() {
				...
				public void itemStateChanged(ItemEvent e) {
					...
					componentConfig.setProperty(keyFromValue,
							(String) e.getItem());
				}
			});
			initCofigOptions(configuraitonItemOptionBox, configurationItem);
			add(configuraitonItemOptionBox, "cell 2 "
					+ nextConfigurationCellRow + ",growx");
			fields.put(configurationItem, configuraitonItemOptionBox);
			nextConfigurationCellRow++;
		}
	}

	private void initCofigOptions(
			final JComboBox<String> configuraitonItemOptionBox,
		...
		configuraitonItemOptionBox.setModel(new DefaultComboBoxModel<String>(
				options));
	}
